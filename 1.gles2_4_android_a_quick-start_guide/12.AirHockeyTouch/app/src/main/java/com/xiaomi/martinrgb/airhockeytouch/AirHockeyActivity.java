...
public class AirHockeyActivity extends AppCompatActivity {
    ...
    private AirHockeyRenderer mAirHockeyRender;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final AirHockeyRenderer mAirHockeyRender = new AirHockeyRenderer(this);

        if(suppoertsEs2){
            //请求 OpenGL ES 2.0的上下文（Context是一个场景,代表与操作系统的交互的一种过程。）
            glSurfaceView.setEGLContextClientVersion(2);
            //分配Renderer
            glSurfaceView.setRenderer(mAirHockeyRender);
            renderSet = true;
        } else {
                    ...
                    if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                        glSurfaceView.queueEvent(new Runnable() {
                            @Override public void run() {
                                mAirHockeyRender.handleTouchPress(normalizedX, normalizedY);
                            }
                        });
                    } else if (motionEvent.getAction() == MotionEvent.ACTION_MOVE) {
                        glSurfaceView.queueEvent(new Runnable() {
                            @Override public void run() {
                                mAirHockeyRender.handleTouchDrag(normalizedX, normalizedY);
                            }
                        });
                    }
                    return true;
