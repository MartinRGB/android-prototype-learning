...
public class CanvasRenderer  implements GLSurfaceView.Renderer{
    ...
    public void onSurfaceChanged(GL10 gl10, int width, int height){
        ...
        //MatrixHelper.perspectiveM(modelViewProjectionMatrix,0,(float) width/(float) height,1f,10f);
        //Matrix.orthoM(modelViewProjectionMatrix,0,-1f,1f,-1f,1f,-1f,1f);

        MatrixHelper.perspectiveM(projectionMatrix,45,(float)width/(float)height,1,10f);


        Matrix.setIdentityM(modelMatrix,0);
        Matrix.translateM(modelMatrix,0,0f,0f,-3f);
        Matrix.rotateM(modelMatrix,0,45f,1f,0f,0f); //45f
